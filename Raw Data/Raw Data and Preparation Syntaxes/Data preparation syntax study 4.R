...
dataimp<-read.csv2("rawdata_study4.csv", stringsAsFactors = FALSE,  na.strings=c("", "NA"))
...
#restore non numeric values to characters
...
mc1<-ifelse(data3$mc_experiments>4, 0, 1)
mc2<-ifelse(data3$mc_rpp>4, 0, 1)
mc3<-ifelse(data3$mc_stats<4, 0, 1)
#count and bind
mcfailure<-mc1+mc2+mc3
dataex0<-cbind(data3,mcfailure)
#exclude with more than 1 failure
dataex1<-subset(data3, mcfailure < 2)
#save(dataex1, file="data_prepared_study4.rda")
#write.csv2(dataex1, file="data_prepared_study4.csv")
